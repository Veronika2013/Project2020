...
Master::Master(QWidget *parent) : MainWindow(parent)
{
    ...
    textDataPass->setGeometry(90, 90, 350, 40);
    textDataPass->show();
    ...
    connect(checkMasterPass, &QPushButton::clicked, this, [=](){
        check_password_in_master_label(*masterWindowTmp ,*lineCheckPassword); //Saving and registration passwords in system
    });
...
void MainWindow::check_password_in_master_label(QLabel &deleteLabelCheck, QLineEdit &password){
  ...
  QString datacomp = settings.value("masterKey", "admin").toString();
   ...
   if(setThemeBo == true){
    ...
    text->setGeometry(400, 40, 170, 40);
    text->showMessage("Your QR-code: ");
    text->show();

    QCommandLinkButton *generatePassBth= new QCommandLinkButton(mainWinApp);
    generatePassBth->setText("Generate passwords");//generate passwords
...
int main(int argc, char *argv[]) {
  ...
  QApplication a(argc, argv);
   QApplication::applicationVersion() = "3.9 Pumpkin";
